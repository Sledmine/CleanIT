...
function guessType(filename)
    ...
    for ext, type in pairs(types) do
        if string.sub(filename, -string.len(ext)) == ext or
            string.sub(filename, -string.len(ext .. ".gz")) == ext .. ".gz" then
            return type
        end
    end
    return "text/plain"
...
function Res:close()
    self._skt:on("sent", function()
    end) -- release closures context
    self._skt:on("receive", function()
    end)
    self._skt:close()
    self._skt = nil
end
    ...
    self._srv:listen(port, function(conn)
        conn:on("receive", function(skt, msg)
            ...
            for i = 1, #self._mids do
                if string.find(req.path, "^" .. self._mids[i].url .. "$") and
                    not self._mids[i].cb(req, res) then
                    break
                end
            end
